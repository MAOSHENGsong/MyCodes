...
def select_device(device='', batch_size=None):
    """
    设备选择与初始化函数，支持自动检测和手动指定

    功能:
    - 自动识别CPU/CUDA设备
    - 设置CUDA_VISIBLE_DEVICES环境变量
    - 验证多GPU训练的batch_size合理性
    - 生成硬件信息日志

    参数:
    - device: 可接受'cpu','0'或'0,1,2'格式的多GPU指定
    - batch_size: 当使用多GPU时验证批量大小可被GPU数量整除

    返回:
    - torch.device对象

    关键细节:
    - 强制CPU模式时会设置CUDA_VISIBLE_DEVICES=-1
    - Windows系统日志输出使用ASCII解码避免表情符号错误
    - 多GPU配置时会检查每个设备的显存容量
    """
    s = f'EfficientTeacher  {git_describe() or date_modified()} torch {torch.__version__} '  # string
