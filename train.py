...
def setup(cfg):
    ...
    if LOCAL_RANK != -1:
        ...
        assert cfg.Dataset.batch_size % WORLD_SIZE == 0, '--batch-size must be multiple of CUDA device count {} VS {}'.format(
            cfg.Dataset.batch_size, WORLD_SIZE)
        # assert not opt.image_weights, '--image-weights argument is not compatible with DDP training'
        torch.cuda.set_device(LOCAL_RANK)
        device = torch.device('cuda', LOCAL_RANK)
        # dist.init_process_group(backend='nccl', init_method='env://', timeout=timeout)  # distributed backend
        dist.init_process_group(backend="nccl" if dist.is_nccl_available() else "gloo", timeout=timeout, rank=RANK,
